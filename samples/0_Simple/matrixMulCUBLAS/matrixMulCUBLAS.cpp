...
void initializeCUDA(int argc, char **argv, int &devID, int &iSizeMultiple,
  ...
  // Hardcode sizes for experiment.
  const int M = 512;
  const int N = 3072;
  const int K = 768;

  matrix_size.uiWA = K;//3 * block_size * iSizeMultiple;
  matrix_size.uiHA = M;//4 * block_size * iSizeMultiple;
  matrix_size.uiWB = N;//2 * block_size * iSizeMultiple;
  matrix_size.uiHB = K;//3 * block_size * iSizeMultiple;
  matrix_size.uiWC = N;//2 * block_size * iSizeMultiple;
  matrix_size.uiHC = M;//4 * block_size * iSizeMultiple;
